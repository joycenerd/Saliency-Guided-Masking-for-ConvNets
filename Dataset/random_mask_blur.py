...
class RandomGridMask(object):
    ...
    def __call__(self, sample, blur_sample, FG_ratio, FG_idx, BG_idx, mask_ratio):
        ...
        for (r,c) in zip(rows, cols):
            sample[:, r * 32:(r+1)*32, c*32:(c+1)*32] = blur_sample[:, r * 32:(r+1)*32, c*32:(c+1)*32]
...
class Masking(object):
    ...
    def __call__(self, imgs, blur_imgs, patches, FG_ratios):
        B = imgs.shape[0]
        ...
        for b in range(B):
            ...
            imgs[b] = self.grid_mask(imgs[b], blur_imgs[b], FG_ratios[b], FG_idx, BG_idx, mask_ratio[b])
